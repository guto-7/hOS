#!/usr/bin/env python3
"""
run_body_composition.py — CLI entry point for the body composition pipeline.
...
Usage:
  python run_body_composition.py <pdf_path>
  python run_body_composition.py <pdf_path> --json-stdout
  python run_body_composition.py <pdf_path> --output-dir ~/Documents/hOS
  python run_body_composition.py <pdf_path> --sex male --age 32
"""
...
from body_composition.unifying import (
    normalise_units,
    resolve_ranges,
    compute_flags,
)
...
def run_pipeline(
    ...
    if output_dir:
        txt_path = Path(output_dir) / "body_composition_extracted.txt"
        txt_path.write_text(extraction.text)

    # Step 4: Parse markers
    if not quiet:
        print("[4/8] Parsing body composition markers...", file=sys.stderr)
    ...
    if not quiet:
        ...
        print(f"       {matched}/{len(scored)} matched to body_composition_markers.json", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="hOS Body Composition Pipeline — Stage 1 (Importing) + Stage 2 (Unifying)"
    )
    ...
    if args.output_dir:
        out = Path(args.output_dir) / "body_composition_enriched.json"
