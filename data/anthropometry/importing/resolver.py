"""
resolver.py — Alias resolution and unit checking against anthropometry_markers.json.

Maps extracted marker names to canonical marker IDs using the
aliases[] arrays in anthropometry_markers.json.
...
MARKERS_JSON = Path(__file__).parent.parent.parent / "anthropometry_markers.json"
...
def load_markers(path: str | Path = MARKERS_JSON) -> list[dict]:
    """Load marker definitions from anthropometry_markers.json."""
    with open(path) as f:
        return json.load(f)
...
def resolve_aliases(
    ...
    """
    Resolve each raw marker against anthropometry_markers.json aliases.
