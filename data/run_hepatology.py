#!/usr/bin/env python3
"""
run_hepatology.py — CLI entry point for the hepatology pipeline.
...
Usage:
  python run_hepatology.py <pdf_path>
  python run_hepatology.py <pdf_path> --json-stdout
  python run_hepatology.py <pdf_path> --output-dir ~/Documents/hOS
  python run_hepatology.py <pdf_path> --sex male --age 34
"""
...
from hepatology.importing import (
    validate_pdf,
    store_pdf,
    extract_text,
    parse_markers,
...
from hepatology.unifying import (
    normalise_units,
    resolve_ranges,
    compute_flags,
)
from hepatology.unifying.ranger import UserProfile
...
def run_pipeline(
    ...
    if output_dir:
        txt_path = Path(output_dir) / "hepatology_extracted.txt"
...
def main():
    parser = argparse.ArgumentParser(
        description="hOS Hepatology Pipeline — Stage 1 (Importing) + Stage 2 (Unifying)"
    )
    ...
    # Save result JSON
    if args.output_dir:
        out = Path(args.output_dir) / "hepatology_enriched.json"
