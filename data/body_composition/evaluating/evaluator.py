"""
evaluator.py — Stage 3 clinical evaluation engine for body composition.
...
Takes the list of FlaggedMarkers (Stage 2 output) and produces:
  ...
  - phenotype: detected body composition phenotype (skinny fat, sarcopenic obesity, etc.)
...
def _score_metabolic_health(
    ...
    Low BMR relative to expected + adverse body composition = high metabolic risk.
    """
...
def _detect_phenotype(markers_by_id: dict, domain_scores: dict[str, DomainScore]) -> Phenotype | None:
    """
    Detect body composition phenotype from domain scores + key marker tiers.
    """
    adip = domain_scores.get("adiposity")
    musc = domain_scores.get("muscularity")
...
def _grade_certainty(markers_by_id: dict) -> tuple[str, str, list[str]]:
    ...
    if len(core_present) == 0:
        return "insufficient", "No core body composition markers found.", missing_for_full
