...
/// Input to the bloodwork Import layer: a PDF file plus optional user profile.
pub struct BloodworkInput {
...
/// Import a bloodwork PDF: save to temp, call run_bloodwork.py for extraction.
pub fn import(input: BloodworkInput) -> Result<RawData, PipelineError> {
    ...
    let script = util::find_script("run_bloodwork.py").map_err(PipelineError::Import)?;
    ...
    let output = cmd
        .output()
        .map_err(|e| PipelineError::ExternalProcess(format!("Failed to run bloodwork script: {e}")))?;
    ...
    if !output.status.success() {
        ...
        return Err(PipelineError::ExternalProcess(format!(
            "Bloodwork extraction failed: {stderr}"
        )));
    }
