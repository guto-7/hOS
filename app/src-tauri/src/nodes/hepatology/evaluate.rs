...
use super::unify::HepatologyData;

/// Evaluate unified hepatology data: surface critical flags, group by category,
/// compute domain scores, match conditions, grade certainty.
pub fn evaluate(data: &HepatologyData) -> Result<EvaluationOutput, PipelineError> {
    ...
    engine_versions.insert("hepatology_evaluator".into(), "1.0.0".into());
...
fn estimate_missing_data(data: &HepatologyData) -> Vec<String> {
    let common_markers = [
